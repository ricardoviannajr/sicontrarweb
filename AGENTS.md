...
- **Skills (Economia de Tokens):** Consultar `SKILLS_SUMMARY.md` (índice leve) para orientação. Para buscas específicas, usar `grep_search` no `SKILLS.md`. **PROIBIDO** ler o `SKILLS.md` inteiro (214KB = ~50K tokens desperdiçados).
- **Memória:** Atualizar **`MEMORIA.md`** em checkpoints de consciência. **Obrigatório:** O arquivo deve ser um **Resumo Executivo Conciso** com no máximo **50 linhas**. Registre o status atual, decisões chave (sem snippets de código) e próximos passos em bullet points curtos. Se ultrapassar 50 linhas, rotacione imediatamente para `session_log/`.
